...
it('Select fills the add input, closes the panel and returns focus to the input', () => {
  let open = true;
...
it('starts closed with Browse beside the labelled path input, not a submit button', () => {
  ...
  expect(html).toContain('type="button" aria-expanded="false" aria-controls="directory-picker"');
  expect(html).toContain('Browse…');
  expect(html).not.toContain('Choose directory');
});
it('announces loading and disables Select until a valid path has loaded', () => {
  const html = render();
...
it('permits selecting an empty directory and suppresses stale folders on errors', () => {
  expect(render({ path: '/media', items: [], next_cursor: null })).toContain(
    'No subfolders — Select to use this path'
  );
  const html = render(
